...
load_datasets <- function(path, date_pattern) {
  # List all files in the directory
  all_files <- dir_ls(path)
  
  # Filter files by the specified date pattern
  filtered_files <- all_files[grepl(date_pattern, all_files)]
  
  # Read each filtered file into a list of data frames
  datasets <- map(filtered_files, read_csv)
  
  # Optional: Assign names to the list elements based on file names
  names(datasets) <- str_replace_all(basename(filtered_files), pattern = "\\.csv$", replacement = "")
...
```{r}
#| code-fold: true
#| code-summary: Load Tidied Datasets
tidied_datasets <- load_datasets("data/tidied", "03-15")
```
...
ui <- fluidPage(
  titlePanel(
    "Nomilo Fishpond Biogeochemical Changes Over Time", 
    windowTitle = "Nomilo Fishpond Biogeochemical Changes Over Time",
    tags$style(HTML(".title {font-weight: bold; text-align: center;}")
    )
  ),
  sidebarLayout(
    sidebarPanel(
...
# ui <- fluidPage(
#   titlePanel("Exploring Depth-wise Biogeochemical Variations Across Nomilo Fishpond Locations Over Time"),
#   sidebarLayout(
#     sidebarPanel(
#       # Dataset selection based on names from tidied_datasets
#       selectInput("dataset", "Choose Dataset:",
#                   choices = c("Select Dataset" = "") + names(tidied_datasets)),
#       uiOutput("y_var_ui"),
#       numericInput("y_min", "Y-axis Minimum:", value = 0),
#       numericInput("y_max", "Y-axis Maximum:", value = 100)
#     ),
...
# 
# server <- function(input, output, session) {
#   # Dynamically update Y variable choices based on selected dataset
#   output$y_var_ui <- renderUI({
#     # Ensure a dataset has been selected
#     if (input$dataset != "Select Dataset") {
#       df <- tidied_datasets[[input$dataset]]
#       num_vars <- names(select_if(df, is.numeric))
#       
#       selectInput("y_var", "Choose Y Variable:",
#                   choices = c("Select Y Variable" = "") + num_vars)
#     } else {
#       selectInput("y_var", "Choose Y Variable:", choices = c("Select Y Variable" = ""))
#     }
#   })
# 
#   # Generate the plot based on selected inputs
#   output$lineplot <- renderPlot({
#     req(input$dataset != "Select Dataset", input$y_var != "Select Y Variable")
#     
#     df <- tidied_datasets[[input$dataset]]
#     formatted_title <- make_title_case(input$y_var)
#     
