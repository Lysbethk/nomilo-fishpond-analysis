...
dfs_no_profiles <- map(files_to_import[c(1:3, 5, 6)], import_list) # <1>

save(dfs_no_profiles, file = "data/raw/2024-03-01_dfs_no_profiles.RData") # <1>
```
...
names(dfs_no_profiles) <- gsub("data/raw/2024-02-28_|\\.xlsx$|\\.xls$", "", files_to_import[c(1:3, 5, 6)]) # <1>
...
ksf_clam_growth_data_tidied <- ksf_clam_growth_data %>%
  ...
  mutate(across(c(count, lbs, avg_per_lbs, growth_in_lbs, growth_pct, sr), ~as.numeric(gsub("%", "", .)))) %>% 
...
oyster_var_names <- c("date", "oyster_large_weight", "oyster_large_gain", "oyster_small_weight", "oyster_small_gain", "oyster_chlorophyll") #<1>
