...
```{r}
# Merging interpolated and completed datasets
...
clams_growth_env_vars_numeric_data <- clams_growth_env_vars_interp %>%
  select_if(is.numeric) %>% 
  select(-days_btwn_clams_sort)

# Compute the correlation matrix
clams_growth_env_vars_corr_matrix <- cor(clams_growth_env_vars_numeric_data)
```
...
ggplot() +
  geom_line(data = clams_growth_env_vars_interp, aes(x = date, y = clams_count, color = "Interpolated")) +
  geom_point(data = clams_growth_env_vars_interp, aes(x = date, y = clams_count, color = "Interpolated")) +
  
  geom_line(data = clams_growth_env_vars_merged, aes(x = date, y = clams_count, color = "Actual")) +
  geom_point(data = clams_growth_env_vars_merged, aes(x = date, y = clams_count, color = "Actual")) +
  
  labs(x = "Date", y = "Number of Clams", title = "Clam Count Over Time", color = "Legend") +
  scale_color_manual(values = c("Interpolated" = "black", "Actual" = "red")) +
  theme_minimal() +
