...
water_samples_data_tidied <- water_samples_data %>%
  ...
  mutate(sample_id = 1:nrow(.)) %>% 
   mutate(across(c(location, tube_name), as.factor),
          date = as.Date(date)) %>% 
   mutate_if(is.numeric, ~replace_na(as.numeric(.), 0))
...
new_profile_var_names <- c("depth", "temperature", "dissolved_oxygen", "salinity", "conductivity", "visibility", "location", "date") 

new_profile_location_col <- c("back buoy", "mid buoy", "production dock", "auwei")

fill_date_col <- c("2023-11-28", "2023-12-22", "2024-01-09", "2024-01-30", "2024-02-20", "2023-11-28", "2023-12-22", "2024-01-09", "2024-01-30", "2024-02-20", "2023-11-28", "2023-12-22", "2024-01-09", "2024-01-30", "2024-02-20", "2023-11-28", "2023-12-22", "2024-01-09", "2024-01-30", "2024-02-20")
fill_date_rows <- c(1:12, 13:24, 25:36, 37:48, 49:57, 58:78, 79:96, 97:114, 115:132, 133:151, 152:159, 160:167, 168:175, 176:183, 184:192, 193:197, 198:202, 203:207, 208:212, 213:214)
...
profiles_data_tidied <- profiles_data %>% 
  ...
  mutate(conductivity = case_when(
    row_number() %in% c(1:9) ~ NA_character_, 
    TRUE ~ conductivity
  ),
  date = ifelse(row_number() %in% fill_date_rows, fill_date_col[match(row_number(), fill_date_rows)], date)
  )
