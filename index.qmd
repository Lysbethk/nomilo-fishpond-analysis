...
profiles_data_tidied <- profiles_data %>%
  ...
  mutate(
    location = ifelse(depth == "Location", temperature, NA_character_),
    date = ifelse(depth == "Date", temperature, NA_character_)
  ) %>%
  fill(location, date, .direction = "down") %>%
  filter(!(depth %in% c("Location", "Date"))) %>%
  mutate(
    location = case_when(
  ...
  mutate(date = as.Date(date),
         ...
         across(c(temperature, dissolved_oxygen, salinity, 
