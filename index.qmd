...
```{r}
#| code-summary: Environmental Variables Merged and Interpolated
#| code-fold: true
continuous_vars <- c(
  "water_temperature", "dissolved_oxygen", "conductivity", "visibility", 
  "chlorophyll_a", "phosphate", "silicate", "nitrate_nitrite", "ammonia", 
  "heterotrophic_bacteria", "large_phytoplankton", "synechococcus_population_1",
  "synechococcus_population_2", "prochlorococcus", "lysbeths_mystery_cells_events", 
  "ksf_rdo_concentration", "ksf_rdo_saturation", "ksf_oxygen_partial_pressure", 
  "ksf_actual_conductivity", "ksf_specific_conductivity", "ksf_salinity", "ksf_density", 
  "ksf_total_dissolved_solids", "ksf_ammonium", "ksf_ammonium_m_v",
  "ksf_barometric_pressure", "pred","outdoor_temperature", "wind_speed_mph", 
  "hourly_rain_inch_hr", "wind_direction", "weight", "growth_pct", "oyster_chlorophyll" 
)
...
env_vars <- reduce(list(profiles_data_tidied, water_samples_data_tidied), full_join, by = c("date", "location", "depth")) %>%
  ...
  mutate(across(any_of(continuous_vars), ~na.fill(na.approx(.x, na.rm = FALSE), "extend"), 
                .names = "interp_{.col}")) %>%
  left_join(reduce(list(ksf_compiled_data_tidied, weather_data_tidied), full_join, by = "date"), 
            by = c("closest_date" = "date")) %>% 
  select(-c(5:20)) %>% 
  na.omit()
...
clams_growth_env_vars_numeric_data <- clams_growth_env_vars_merged %>% select_if(is.numeric)

# Compute the correlation matrix
clams_growth_env_vars_corr_matrix <- cor(clams_growth_env_vars_numeric_data, use = "complete.obs")
```

```{r}
#| echo: false
kable(clams_growth_env_vars_corr_matrix, caption = "KSF Clams Growth and Environmental Variables Correlation Matrix") %>%
...
#| fig-width: 20
#| fig-height: 20
corrplot(clams_growth_env_vars_corr_matrix, method = "circle",
...
#| echo: false
# Select only numeric columns for correlation analysis
oyster_growth_env_vars_numeric_data <- oyster_growth_env_vars_merged %>% select_if(is.numeric)

# Compute the correlation matrix
oyster_growth_env_vars_corr_matrix <- cor(oyster_growth_env_vars_numeric_data, use = "complete.obs")
```

```{r}
#| echo: false
kable(oyster_growth_env_vars_corr_matrix, caption = "KSF Oyster Cylinder Growth and Environmental Variables Correlation Matrix") %>%
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover"))
```
...
#| fig-width: 20
#| fig-height: 20
corrplot(oyster_growth_env_vars_corr_matrix, method = "circle",
         tl.cex = 2, # Text label size, change as needed
         tl.col = "black", # Text label color
         tl.srt = 90, # Text label rotation in degrees
         tl.offset = 0.5, 
