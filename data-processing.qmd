...
```{r}
#| code-fold: true
#| code-summary: "Step 2: Define vector of files to import"
#| warning: false
...
#| warnings: false
#| code-fold: true
#| code-summary: "Step 3: Only select .xlsx files"
...
#| eval: false
#| code-fold: true
#| code-summary: "Step 4a: Use the `purrr::map()` function to iteratively import files in the `files_to_import` vector except for the profiles data and .RData files"
dfs_no_profiles <- map(xlsx_files, import_list) # <1>
current_date <- format(Sys.Date(), "%Y-%m-%d")
save(dfs_no_profiles, file = paste0("data/raw/", current_date, "_dfs-no-profiles.RData")) # <1>
```

::: {.callout-caution title="Step 4a Execution Warning"}
Step 4a should only be ran once when raw data is updated
because it takes long to execute. Therefore, do step 4b instead to easily import up-to-date raw data.
:::

```{r}
#| label: efficiently-load-raw-data
#| code-fold: true
#| code-summary: "Step 4b: Efficiently import up-to-date raw data"
base::load(files_to_import[11])
```
...
#| code-fold: true
#| code-summary: "Step 5: Rename datasets"
...
#| code-fold: true
#| code-summary: "Step 6: Rename each sheet within each raw dataset to be lowercased and replace spaces with underscores"
...
#| code-fold: true
#| code-summary: "Step 7: Create separate datasets by specifying the Excel sheet from each spreadsheet we want to tidy"
...
#| code-fold: true
#| code-summary: "Step 8: We want to combine multiple sheets within the profiles Excel spreadsheet into one, therefore, we will import it separately"
sheets_to_import <- c("L1", "L2", "L3", "L4") # <1>

profiles_data <- profiles_data <- map_dfr(sheets_to_import, function(sheet_name) { # <2>
  import(files_to_import[4], which = sheet_name) # <2>
}) %>%  # <2>
  bind_rows() # <3>
```
...
new_clam_date_col <- c( #<2>
  ...
  "2024-01-31", "2024-02-08", "2024-02-13", "2024-03-14", "2024-03-19", "2024-03-27" #<2>
  ) #<2>
...
#| eval: false
#| code-fold: true
#| code-summary: Export tidied datasets to CSV into `data/tidied` folder
...
```{r}
#| code-fold: true
#| code-summary: Export merged final data set into `data/outputs` folder
#| eval: false
